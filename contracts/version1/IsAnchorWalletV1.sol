...
contract IsAnchorWalletV1 {
	LamportWalletFactory public factory;

	constructor(LamportWalletFactory _factory) {
		factory = _factory;
	}

	function isAnchorWallet(address _wallet) external view returns (bool) {
    ...
    function walletNumber(address _wallet) external view returns (uint256) {
        return factory.wallet_address_to_wallet_number(_wallet);
    }

    function walletNumberToData(uint256 num) external view returns (address, uint256) {
        return factory.wallets(num);
    }
